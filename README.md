...
- It was discovered that when payment was successfully made an email was meant to be sent to the user with details of their order however this was not happening. It was found that a line of code had been left int the webhooks.py file during development which gave an early return statement and meant that the webhook wasn't propley created and didn't reach the lines of code to create the email. Once the line of code was removed an error appeared statiing that the required template files to send the email didn't exist but this was found to be due to a typo and was quickly fixed.
